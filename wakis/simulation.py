...
class WarpX():
	'''Mixin class for optimized WarpX simulations
	'''
	def setup_params(self, **kwargs):
		...
		try:	
			...
			self.log.warning('check WarpX installation')
			raise ImportError('pywarpx could not be imported')
		...
		# beam parameters
		self.q = 1e-9                   #Total beam charge [C]
		self.sigmaz = 5*UNIT            #[m]
		...
		self.log.info('Initialized mesh with ' + str((nx,ny,nz)) + ' number of cells')
		...
		self.log.info('Timesteps to simulate = '+ str(max_steps) + ' with timestep dt = ' + str(dt))
		self.log.info('Wake length = '+str(Wake_length/UNIT)+ ' mm')
	...
	def beam_injection(self, **kwargs):
