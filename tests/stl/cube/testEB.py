...
#=========================#
# Set up WarpX simulation #
#=========================#

UNIT = 1e-2
warpx = wakis.WarpX(wake_length=10.)

# add geometry
warpx.set_geometry(stl_file='cube.stl', stl_scale=1.0e-2, stl_reverse_normal=True,
                   xmin=-35.5*UNIT, xmax=+35.5*UNIT, ymin=-35.5*UNIT, ymax=+35.5*UNIT,
                   zmin=-30.5*UNIT, +30.5*UNIT)

# configure mesh
warpx.set_mesh(dh=1.0*UNIT)
...
plt.ion()
if fcontour:
    fig_contour, axs = plt.subplots(1,3,tight_layout=True, figsize=(12,5)) 
if fplot:
    fig_plot, ax = plt.subplots(1,1,tight_layout=True, figsize=(8,5)) 
...
for n in range(max_steps-100):
    ...
    if Ez.any() and fcontour:
        ...
        contourEz(xf, yf, Ez[:,:,int(len(zf)/2)], ax=axs[0], fig=fig_contour, title='Ez(x,y)')
        contourEz(xf, zf, Ez[:,int(len(yf)/2), :], ax=axs[1], fig=fig_contour, title='Ez(x,z)') 
        contourEz(yf, zf, Ez[int(len(xf)/2), :, :], ax=axs[2], fig=fig_contour, title='Ez(y,z)')
        fig.suptitle(f'E field at timestep = {n}')
        fig.canvas.draw()
        fig.canvas.flush_events()

    if Ez.any() and fplot:
        plotEz(zf, Ez[int(len(xf)/2), int(len(yf)/2), :], rho[int(len(xf)/2), int(len(yf)/2), :], fig=fig_plot)
        fig.suptitle(f'E field at timestep = {n}')
        fig.canvas.draw()
        fig.canvas.flush_events()
