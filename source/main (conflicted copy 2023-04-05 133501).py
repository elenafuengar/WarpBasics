...
if case == 'cst':
    ...
    path = '/eos/user/e/edelafue/data/cubpillbox/'
    # set unit conversion [*m] to [m]
    unit = 1e-2 #1e-3 

    # Beam parameters
    sigmaz = 5*unit #15*unit                  #beam longitudinal sigma
...
s = np.arange(-t_inj*c, Wake_length, dt*c) 
...
with tqdm(total=3*3*len(s)) as pbar:
    ...
    for i in range(-i0,i0+1,1):  
        for j in range(-j0,j0+1,1):
            ...
            for n in range(nt):
                Ez=hf.get(dataset[n])
                Ezz[:, n]=Ez[Ez.shape[0]//2+i,Ez.shape[1]//2+j,:]

            #integral of (Ez(xtest, ytest, z, t=(s+z)/c))dz
            for n in range(len(s)):   
                for k in range(0, nz): 
                    ts=(z[k]+s[n])/c-zmin/c-t[0]+t_inj
                    it=int(ts/dt)                   #find index for t
                    WP[n]=WP[n]+(Ezz[k, it])*dz     #compute integral
...
ax.plot(s/unit, lambdas*max(WP)/max(lambdas), c='r', lw=1., label= '$\lambda$(s)')
ax.plot(s/unit, WP, lw=2., c='darkorange', label = 'WPz(s) Wakis')
