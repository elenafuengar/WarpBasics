...
def getBoundingBox(path, fname, unit): 
    ''' Gets geometry model bounding box from .stl
    '''
    from stl import mesh
    obj = mesh.Mesh.from_file(path+fname)
    W = (obj.x.max() - obj.x.min())*unit
    H = (obj.y.max() - obj.y.min())*unit
    L = (obj.z.max() - obj.z.min())*unit
